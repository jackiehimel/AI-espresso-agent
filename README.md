...
AI Espresso generates a daily AI news edition: 3-6 curated story cards plus one "Try this prompt" card, rendered for web/email distribution.

## Features

- Hybrid discovery pipeline: LLM ranking of the full candidate pool, then an agentic Editor tool-use loop with deterministic ship gates
- Cross-edition deduplication (canonical URL, normalized title, and embedding similarity) so stories never repeat across editions
- Source ingestion from a tiered catalog in `agent/sources.yaml`, plus web-search discovery
...
└── .github/workflows/          # daily edition cron, CI tests, Docker publish
```
